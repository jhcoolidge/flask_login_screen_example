...
def get_error_report(errors, file):
    report = []
    

    for i in range(0,len(errors)):
        #print('Error # - {}  Error Message - {}'.format(i,errors[i]['message']))
        try:
            error_location = re.findall(r'\d+', errors[i]['message'])
            print(type(file))
            file.seek(int(error_location[0]))
        #    print('Input data - {}'.format(file.readline()))
            data = file.readline()
            log_string = """| Error # - {} || Error Message - {} || Inputted Data - {} |
            """.format(i, errors[i]['message'], data)
                    ...
                    try:
                        ...
                        try:
                            ...
                            if load_job.errors:
                                ...
                                log = get_error_report(load_job.errors,file)
